...
@selector(
    uses={"Jet.pt", "Jet.eta", "Jet.phi", "Jet.mass", "Jet.btagDeepFlavB", "Jet.btagCSVV2", "Jet.jetId"},
...
def jet_selection(
    ...
    **kwargs,
    ...
    jet_mask = (
        (events.Jet.pt > 20) & (abs(events.Jet.eta) < 2.4) & #(events.Jet.jetId == 6) & #jetId == 6 ak4? 
    ...
    bjet1, bjet2 = ak.unzip(bjet_pairs)
    bjet_pairs["combinedVertex"] = (bjet1.btagCSVV2 + bjet2.btagCSVV2)
    chosen_bjet_pair = bjet_pairs[ak.singletons(ak.argmax(bjet_pairs.combinedVertex, axis=1))]
    bjet1, bjet2 = [chosen_bjet_pair[i] for i in ["0","1"]]
...
@selector(
    uses={
        ...
        "Electron.charge", "Electron.pdgId",
        "Electron.cutBased", "Electron.mvaFall17V2Iso_WP80",
        "Muon.mediumId", "Muon.looseId", "Muon.pfRelIso04_all",
        "Muon.pt", "Muon.eta", "Muon.phi", "Muon.mass",
        "Muon.charge", "Muon.pdgId",
        "Tau.pt", "Tau.eta",
     },
...
def dilep_selection(
    ...
    e_mask_loose = (
        ...
        (electron.pt > 15) &
        #(electron.cutBased == 3) & 
        #(electron.pfRelIso03_all < 0.04) &
#TODO: is this the Isolation or off(onine distinguish?         
        (electron.mvaFall17V2Iso_WP80 == 1) 
    )
    ...
    # saving electron and muon indices     
    e_indices = masked_sorted_indices(e_mask_loose, electron.pt)
    mu_indices = masked_sorted_indices(mu_mask_loose, muon.pt) 
    ##__import__("IPython").embed()
    ...
    #pdgId = ak.fill_none(ak.pad_none(leptons.pdgId, 2, axis=-1), -999)
    #charge = ak.fill_none(ak.pad_none(leptons.charge, 2, axis=-1), -999)
    #e_idx = ak.fill_none(ak.pad_none(leptons.e_idx, 2, axis=-1), -999)
    #mu_idx = ak.fill_none(ak.pad_none(leptons.mu_idx, 2, axis=-1), -999)
    #leptons = ak.zip({
    #    "pt": pt, "eta": eta, "phi": phi, "mass": mass, "charge": charge, 
    #    "pdgId": pdgId, "e_idx": e_idx, "mu_idx": mu_idx,
    #}) 
    fill_with = {
    ...
    # save channel Id and m_ll as features 
    leptons['channel_id'] = channel_id
    leptons['m_ll'] = invariant_mass(leptons)
...
@dilep_selection.init
def dilep_selection_init(self: Selector) -> None:
    ...
    if not self.emu_trigger:
        self.emu_trigger = {
            2017: "Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL" or "Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ"
        }[year]
        self.uses.add(f"HLT.{self.emu_trigger}")
    if not self.mue_trigger:
        self.mue_trigger = {
            2017: "Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL" or "Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL"
        }[year]
